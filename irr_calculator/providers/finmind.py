...
class FinMindProvider:
    ...
    def __init__(self, token: str, timeout: float = 15.0, transport: httpx.BaseTransport | None = None) -> None:
    ...
    def _request(self, params: dict[str, str]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ProviderError(self._safe(f"FinMind request failed: {error}")) from error
        if not isinstance(payload, dict) or payload.get("status") != 200:
            message = payload.get("msg", "malformed response") if isinstance(payload, dict) else "malformed response"
