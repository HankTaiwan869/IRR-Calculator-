...
Reimplement the original Streamlit calculator as a fast-starting, local PyQt6 desktop application for Taiwan securities. Preserve XIRR and projections while adding named portfolios, signed-share transactions, a locally searchable security master, manually refreshed daily prices, moving-average cost basis, auditable corrections, and explicit handling for fully reinvested dividends.

The interface is English-only. Chinese security names remain searchable provider data, not interface translations. Use a persistent left navigation sidebar with a modern Qt desktop layout, clear visual hierarchy, and comfortable typography.
...
- Use `Decimal` at accounting boundaries. Convert to `float` only where required by pyxirr or charting libraries.
...
1. Replace Streamlit with a lightweight `main.py` launcher and packaged `irr_calculator` application. Use PyQt6, SQLAlchemy, pyxirr, httpx, keyring, matplotlib or PyQtGraph, and pytest. Remove Streamlit, pandas, CustomTkinter/Tkinter dependencies, and duplicate dependency files. Keep `uv` as the dependency and run workflow.
...
3. Introduce a versioned SQLite schema with `portfolios`, `securities`, `transactions`, `quotes`, `transaction_audit`, `schema_meta`, and `import_runs`. Enable SQLite foreign keys on every connection. Enforce unique portfolio names, unique provider/security symbols, soft deletion, indexed transaction filters, positive quote constraints, and decimal monetary/share fields. Derive holdings from transactions instead of storing a second mutable holdings total.

4. Define `transactions` around separate accounting concepts: `kind`, signed `shares_delta`, signed `external_cash_flow`, nonnegative `trade_amount`, nonnegative `income_amount`, optional `unit_price` and `fees`, dates, portfolio/security references, notes, timestamps, and `deleted_at`.
...
7. Add a dedicated `REINVESTED_DIVIDEND` transaction workflow. Record net dividend as `income_amount`, acquired shares as positive `shares_delta`, and actual purchase cost as `trade_amount`; derive/display unit price and support optional fees. For a fully reinvested dividend, set `external_cash_flow` to zero. Increase moving-average basis by acquisition cost plus fees, count the dividend as income, and exclude it from XIRR external flows. If dividend cash and purchase cost differ, require the residual to be classified as a paid-out remainder or owner top-up.
...
11. Sync the security master into SQLite on first use or from Settings, never during blocking startup. Search locally by partial code or Chinese name. Implement debounced search with `QTimer` and optional exchange/type filters.
...
13. Build the application shell around `QMainWindow`. Use:
   ...
   Navigation items are Dashboard, Transactions, History, Portfolios, and Settings & Import. Give the active item a clear selected state. Preserve the selected portfolio and relevant page state when switching views.
...
16. Build **Transactions** as a structured form rather than a dense grid. Use `QComboBox`/search completion for portfolio and security, `QDateEdit` for dates, numeric editors with validation, and clear form sections. Provide distinct transaction modes for regular activity and **Reinvested Dividend** so the compound accounting behavior is visible rather than hidden.
...
19. Build **Settings & Import** as grouped settings sections rather than one long form. Include FinMind token save/test, security-master sync status, legacy import/reconciliation, database backup, appearance options, and projection assumptions. Use `QFileDialog`, `QMessageBox`, progress dialogs/bars, and native Qt controls. Do not add language preferences or localization infrastructure.
...
23. Add focused pytest coverage for schema constraints, portfolio isolation, local search, quote caching/fallback, provider error normalization/token redaction, audit edits/deletes/restores, legacy import idempotency, opening-position reconciliation, moving-average buys/sells/closed positions, combined-dashboard isolation, and IRR/profit/projection edge cases. Include a reinvested-dividend test proving that shares, basis, and dividend income increase while external cash flow remains zero and XIRR receives no artificial contribution/distribution.
...
- `irr_calculator/ui/views/*`: Dashboard, Transactions, History, Portfolios, and Settings & Import.
...
4. Manually exercise every navigation page: create/rename/archive/delete-empty portfolios; search Chinese security names and codes; add regular and reinvested transactions; edit/delete/restore with audit history; refresh partial-failure quotes; and compare portfolio versus all-portfolios totals.
...
- Interface and documentation are English-only. Chinese security names remain searchable market data.
...
- Included: local single-user desktop app, named portfolios, Taiwan security search, delayed daily price refresh, quote cache, signed-share transaction CRUD, reinvested cash dividends, moving-average basis, audit history, portfolio/all-portfolio analytics, legacy migration, English-only PyQt6 UI, tests, and documentation.
