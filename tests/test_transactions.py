...
def buy(portfolio_id, security_id, day=date(2025, 1, 1), shares=10, amount=100):
    return TransactionInput(portfolio_id, security_id, TransactionKind.BUY, day, shares, -amount, amount)
...
def test_signed_rules_and_backdated_negative_holdings(db):
    ...
    with factory.begin() as session:
        ...
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.SELL, date(2025, 2, 1), -5, 60, 60))
    with pytest.raises(ValidationError), factory.begin() as session:
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.SELL, date(2024, 12, 1), -1, 12, 12))
...
def test_integer_like_inputs_are_normalized_to_builtin_ints(db):
    _engine, factory, (portfolio_id, security_id) = db
    with factory.begin() as session:
        row = create_transaction(session, TransactionInput(
