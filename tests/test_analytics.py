...
from irr_calculator.services.analytics import calculate_xirr, portfolio_summary, projection
...
def test_reinvested_dividend_increases_shares_basis_and_income_without_xirr_flow(db):
    ...
    with factory.begin() as session:
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 10, -100, 100))
        reinvested = create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.REINVESTED_DIVIDEND, date(2024, 7, 1), 1, 0, 10, 10))
        assert reinvested.external_cash_flow == 0
        session.add(Quote(security_id=security_id, market_date=date(2025, 1, 1), refresh_cycle_date=date(2025, 1, 1), close=11))
...
def test_closed_position_keeps_realized_profit_and_dividends(db):
    ...
    with factory.begin() as session:
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 10, -100, 100))
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.DIVIDEND, date(2024, 6, 1), 0, 5, income_amount=5))
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.SELL, date(2024, 12, 1), -10, 130, 130))
    with factory() as session:
        summary = portfolio_summary(session, date(2025, 1, 1), portfolio_id)
...
def test_moving_average_cost_uses_integer_half_up_allocation(db):
    _engine, factory, (portfolio_id, security_id) = db
    with factory.begin() as session:
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 3, -100, 100))
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.SELL, date(2024, 2, 1), -1, 50, 50))
    with factory() as session:
        ledger = replay_ledger(list(session.query(Transaction).all()))
...
def test_ledger_and_projection_outputs_are_integers_with_half_up_rounding(db):
    _engine, factory, (portfolio_id, security_id) = db
    with factory.begin() as session:
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 2, -1, 1))
    with factory() as session:
        ledger = replay_ledger(list(session.query(Transaction).all()))
...
def test_xirr_not_calculable_without_sign_change():
    row = Transaction(kind=TransactionKind.LEGACY_CASH_FLOW, trade_date=date(2024, 1, 1), external_cash_flow=5, shares_delta=0, trade_amount=0, income_amount=0, portfolio_id=1)
...
def test_all_portfolios_replays_separate_ledgers_before_aggregating(db):
    ...
    with factory.begin() as session:
        ...
        session.add(second)
        session.flush()
        create_transaction(session, TransactionInput(first_portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 10, -100, 100))
        create_transaction(session, TransactionInput(second.id, security_id, TransactionKind.BUY, date(2024, 2, 1), 10, -200, 200))
        create_transaction(session, TransactionInput(first_portfolio_id, security_id, TransactionKind.SELL, date(2024, 6, 1), -5, 75, 75))
        session.add(Quote(security_id=security_id, market_date=date(2025, 1, 1), refresh_cycle_date=date(2025, 1, 1), close=20))
...
def test_transactions_after_valuation_date_are_excluded_from_holdings_and_xirr(db):
    ...
    with factory.begin() as session:
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 10, -100, 100))
        create_transaction(session, TransactionInput(portfolio_id, security_id, TransactionKind.BUY, date(2026, 1, 1), 100, -1000, 1000))
        session.add(Quote(security_id=security_id, market_date=date(2025, 1, 1), refresh_cycle_date=date(2025, 1, 1), close=11))
    ...
    assert summary.annual_irr is not None
    assert abs(summary.annual_irr - .1) < .001
...
def test_all_portfolios_excludes_archived_but_explicit_selection_still_works(db):
    ...
    with factory.begin() as session:
        ...
        session.flush()
        create_transaction(session, TransactionInput(active_portfolio_id, security_id, TransactionKind.BUY, date(2024, 1, 1), 10, -100, 100))
        create_transaction(session, TransactionInput(archived.id, security_id, TransactionKind.BUY, date(2024, 1, 1), 90, -900, 900))
        session.add(Quote(security_id=security_id, market_date=date(2025, 1, 1), refresh_cycle_date=date(2025, 1, 1), close=10))
