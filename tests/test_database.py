...
def test_v1_decimal_schema_migrates_to_integer_columns_and_values(tmp_path):
    ...
    with sqlite3.connect(path) as connection:
        ...
        assert {
            row[2] for row in connection.execute("PRAGMA foreign_key_list(quotes)")
        } == {"securities"}
        assert {
            row[2]
            for row in connection.execute("PRAGMA foreign_key_list(transaction_audit)")
        } == {"transactions"}
        ...
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "UPDATE transactions SET portfolio_id = 999 WHERE id = 1"
            )
        connection.rollback()
